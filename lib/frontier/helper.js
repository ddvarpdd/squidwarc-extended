...
const { cmodePO, cmodePAL, cmodePSD, cmodSite, cmodePSSD, cmodePSP } = require('./modes')
...
class FrontierHelper {
   ...
   * @returns {boolean}
   */
  static shouldAddToFrontier (url, curURL, tracker, samePathRegexp) {
    if (tracker.mode === cmodePSD || tracker.mode === cmodSite || tracker.mode === cmodePSSD || tracker.mode === cmodePSP) {
	  let decision = FrontierHelper.shouldAddToFrontierPSD(url, curURL, tracker, samePathRegexp)
	  Logger.logFrontierDecision(url.href, curURL, decision)
	  return decision
    }
    let decision = FrontierHelper.shouldAddToFrontierDefault(url, curURL, tracker)
	Logger.logFrontierDecision(url.href, curURL, decision)
	return decision
  }
   ...
   * @returns {boolean}
   */
  static shouldAddToFrontierPSD (url, curURL, tracker, samePathRegexp) {
    const cDomain = parseDomain(curURL)
    const ext = Path.extname(url.pathname)
    const td = parseDomain(url.host)
    let tdTest = false
	if(tracker.mode === cmodePSSD && cDomain != null){
		tdTest = td && cDomain.domain === td.domain && cDomain.subdomain === td.subdomain
	} else if(tracker.mode === cmodePSP){
		tdTest = samePathRegexp.test(url.href)
		// for testing purpose
		/*const logObj = fs.readJsonSync('./log.json')
		logObj.push({ url: url.href, test: tdTest })
		fs.outputJsonSync('./log.json', logObj, {spaces: '\t'})*/
		// end for testing purpose
	} else {
		tdTest = td && cDomain.domain === td.domain
	}
    if (ext !== '') {
	  if(!tdTest || bigExtLookup[ext]){
		Logger.logUnselectedExtractedLink(url.href)
	  }
