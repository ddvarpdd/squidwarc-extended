...
class Frontier {
  ...
  constructor () {
	...
	/**
     * @desc Used to log processed items count
     * @type {number}
     */
    this.processed = 0
	
	/**
	 * @desc Store regexp for same path mode
	 * @type {Regexp}
	 */
	this.seedUrlWithRootPathRegexp = null
  }
  ...
  init (starting) {
    if (Array.isArray(starting)) {
      ...
      while (i < len) {
		...
		if(strt.mode === cmodePSP){
		  this.seedUrlWithRootPathRegexp = new RegExp("^" + strt.url)
		}
        i++
      }
    } else {
  ...
  /**
   * @desc Returns the number of processed items
   * @return {number}
   */
  getProcessed () {
  ...
  process (links) {
    ...
    if (this.current.mode !== cmodePO) {
      ...
      while (i--) {
        url = links[i]
        if (FH.shouldAddToFrontier(url, this.current.url, tracker, this.seedUrlWithRootPathRegexp)) {
  ...
  _processSiteMode (links) {
	...
	Logger.logInfo("Adding " + tmpLoggerValue + " URL to queue")
  }
