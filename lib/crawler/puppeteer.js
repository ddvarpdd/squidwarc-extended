...
const screenShotter = require('../utils/getScreenshot')
...
class PuppeteerCrawler extends EventEmitter {
  ...
  async navigate (url) {
    ...
    try {
      ...
      Logger.reportError(this._currentUrl)
      return false
    }
    this._pages.push(this._currentUrl)
    return true
  ...
  async runUserScript () {
    ...
    if (this.options.screenshot.enabled) {
        try {
            await screenShotter(this._page, this.options.screenshot.output)
        } catch (e) {
            cp.error('Error getting screenshot', e)
            //Logger.logError("Error while running the default scroll script (" + e + ") for URL " + this._currentUrl)
        }
    }
